...
const Profile = () => {
  ...
  const fetchImages = () => {
    fetch(`http://localhost:8082/userPosts/${email}`, {
      method: "GET",
    })
      .then((res) => {
        return res.json();
      ...
      .then((res) => {
        if (res.length == 0) {
          fetch(`http://localhost:8082/userByemail/${email}`, {
            method: "GET",
          })
            .then((x) => {
              return x.json();
        ...
        temped = [];
        temped2 = [];
        fetch(`http://localhost:8082/following/${res[0].user.id}`, {
          method: "GET",
        })
          .then((x) => {
            return x.json();
          ...
          if (i == res.length - 1) {
            ...
            temped.forEach((c, j) => {
              ...
              if (j == temped.length - 1) {
                if (temped2.length == 0 || post.length == 0) {
                  setPost([...theAuthor]);
                } else {
                  setPost(temped2);
                  console.log(temped2);
  ...
  const follow = (id) => {
    fetch(
      `http://localhost:8082/follow/${id}/${localStorage.getItem("username")}`,
      {
        method: "PUT",
      ...
      .then((res) => {
        ...
        if (res.message == "ALREADY_FOLLOWED") {
          fetch(
            `http://localhost:8082/unfollow/${id}/${localStorage.getItem(
              "username"
            )}`,
            {
              method: "DELETE",
